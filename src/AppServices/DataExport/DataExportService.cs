    ...
    IMemoryCache cache) : IDataExportService
{
        ...
        CancellationToken token)
    {
        ...
        await CreateDataExportFileAsync(exportMeta, exportFilePath, token).ConfigureAwait(false);
        return exportMeta;
    }
        ...
        CancellationToken token)
    {
        ...
        await fileService.SaveFileAsync(zipMemoryStream, exportMeta.FileName, exportFilePath, token)
            .ConfigureAwait(false);
    }

    private async Task DeleteOldExportFilesAsync(string exportFilePath, CancellationToken token)
    {
        var filesAsyncEnumerable = fileService.GetFilesAsync(exportFilePath, token);
