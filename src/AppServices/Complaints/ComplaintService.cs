    ...
    : IComplaintService
{
    public async Task<ComplaintPublicViewDto?> FindPublicAsync(int id, CancellationToken token = default)
    {
        var complaint = await complaintRepository
            .FindIncludeAllAsync(ComplaintFilters.PublicIdPredicate(id), token: token);
        ...
        ComplaintPublicSearchDto spec, PaginatedRequest paging, CancellationToken token = default)
    {
        ...
        var count = await complaintRepository.CountAsync(predicate, token);

        var list = count > 0
            ? mapper.Map<List<ComplaintSearchResultDto>>(
                await complaintRepository.GetPagedListAsync(predicate, paging, token))
    ...
    {
        var attachment = await complaintRepository.FindAttachmentAsync(id, token);
    ...
    public async Task<bool> ExistsAsync(int id, CancellationToken token = default) =>
        await complaintRepository.ExistsAsync(id, token);
    ...
    public async Task<int> CreateAsync(ComplaintCreateDto resource, CancellationToken token = default)
    {
        ...
        await complaintRepository.InsertAsync(complaint, autoSave: false, token: token);
        ...
        await complaintRepository.SaveChangesAsync(token);
        return complaint.Id;
    }

    public async Task UpdateAsync(int id, ComplaintUpdateDto resource, CancellationToken token = default)
    {
        var complaint = await complaintRepository.GetAsync(id, token);
        complaint.SetUpdater((await userService.GetCurrentUserAsync())?.Id);
        await FillInComplaintDetailsAsync(complaint, resource, token);
        await complaintRepository.UpdateAsync(complaint, token: token);
    }
        ...
        ComplaintCreateDto resource, string? currentUserId, CancellationToken token)
    {
        ...
        if (!string.IsNullOrEmpty(currentUserId))
            complaint.EnteredBy = await userService.GetUserAsync(currentUserId);
        ...
        complaint.CurrentOwner = await userService.FindUserAsync(resource.CurrentOwnerId);
        ...
        CancellationToken token)
    {
        ...
        complaint.ReceivedBy = await userService.GetUserAsync(resource.ReceivedById!);
...
#pragma warning restore S125
    ...
    {
        var complaint = await complaintRepository.GetAsync(complaintId, token);
        complaint.SetDeleted((await userService.GetCurrentUserAsync())?.Id);
        await complaintRepository.UpdateAsync(complaint, token: token);
    }
    ...
    public async ValueTask DisposeAsync()
    {
        ...
        await concernRepository.DisposeAsync();
