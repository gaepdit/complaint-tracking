...
public class ConcernSeedData : IDataSeedContributor, ITransientDependency
{
    private readonly IRepository<Concern, Guid> _repository;

    public ConcernSeedData(IRepository<Concern, Guid> repository) => _repository = repository;

    public async Task SeedAsync(DataSeedContext context)
    {
        if (await _repository.GetCountAsync() <= 0) await _repository.InsertManyAsync(ConcernData(), autoSave: true);
    }

    private static IEnumerable<Concern> ConcernData()
    {
        string[] items =
        {
        ...
        return items.Select(i => new Concern { Name = i, Active = true });
    }
}
